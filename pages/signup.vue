<template>
  <div class="flex items-center justify-center bg-gradient-to-b from-blue-200 to-blue-50">
    <v-sheet class="px-8 w-full max-w-lg" elevation="5" style="background-color: #ffffff; border-radius: 20px;">
      <div class="flex items-center ">
        <img src="/icon.jpg" class="w-36 h-36 rounded-full  " alt="Logo">
        <h1 class="text-blue-800 font-bold text-5xl my-2 mx-1 flex items-center">Sign Up</h1>
      </div>

      <h1 class="text-xl mb-5 mx-5 flex items-center">
      ...
      <v-form @submit.prevent="submitSignUp">
        ...
        <v-text-field v-model="user.username" label="ชื่อผู้ใช้" variant="outlined" outlined></v-text-field>
       
        <p v-if="errorMessages.firstname" class="text-red-600 mx-5">{{ errorMessages.firstname }}</p>
        <v-text-field v-model="user.firstname" label="ชื่อจริง" variant="outlined" outlined></v-text-field>

        <p v-if="errorMessages.lastname" class="text-red-600 mx-5">{{ errorMessages.lastname }}</p>
        <v-text-field v-model="user.lastname" label="นามสกุล" variant="outlined" outlined></v-text-field>
       
        <p v-if="errorMessages.email" class="text-red-600 mx-5">{{ errorMessages.email }}</p>
        <v-text-field v-model="user.email" label="อีเมล" variant="outlined" outlined></v-text-field>
        
        <p v-if="errorMessages.phone_number" class="text-red-600 mx-5">{{ errorMessages.phone_number }}</p>
        <v-text-field v-model="user.phone_number" label="เบอร์โทรศัพท์" variant="outlined" outlined></v-text-field>
        
        <p v-if="errorMessages.department" class="text-red-600 mx-5">{{ errorMessages.department }}</p>
        <v-text-field v-model="user.department" label="แผนกงาน" variant="outlined" outlined></v-text-field>
        
        <p v-if="errorMessages.password" class="text-red-600 mx-5">{{ errorMessages.password }}</p>
        <v-text-field v-model="user.password" label="รหัสผ่าน" type="password" variant="outlined" outlined></v-text-field>
        
        
        <v-checkbox v-model="agreeToPolicy" label="ฉันยอมรับบริการและนโยบายความเป็นส่วนตัว"></v-checkbox>
        <p v-if="!agreeToPolicy && showValidationMessage" class="text-red-600 mx-5">คุณต้องยอมรับบริการและนโยบายความเป็นส่วนตัวก่อนสมัคร</p>
        ...
        <div class="flex items-center mb-6">
          <h1 class="mx-3 text-base">ถ้าคุณมีสมาชิกอยู่แล้ว? </h1>
